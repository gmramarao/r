...
export class AuthService {
    ...
    //   return this.http.post('http://localhost:3000/admin/register', user, { headers: header }).map(res => res.json());
      return this.http.post('https://api.reatchall.com/admin/register', user, { headers: header }).map(res => res.json());
    }
    authenticateUser(user) {
        const header = new Headers();
        header.append('Content-Type', 'application/json');
        // return this.http.post('http://localhost:3000/admin/authenticate', user, { headers: header }).map(res => res.json());
        return this.http.post('https://api.reatchall.com/admin/authenticate', user, { headers: header }).map(res => res.json());
    }
    authenticateMobile(mobile: string) {
    //   return this.http.get('http://localhost:3000/admin/find-mobile/' + mobile).map(res => res.json());
      return this.http.get('https://api.reatchall.com/admin/find-mobile/' + mobile).map(res => res.json());
    }
    authenticateEmail(email: string) {
    //   return this.http.get('http://localhost:3000/admin/find-email/' + email).map(res => res.json());
      return this.http.get('https://api.reatchall.com/admin/find-email/' + email).map(res => res.json());
    }
    getUserFromLocal() {
    ...
    updateUser(user) {
        ...
        return this.http.post('https://api.reatchall.com/admin/update-user', user, { headers: header }).map(res => res.json());
    }
    updatePassword(pwd) {
        const header = new Headers();
        header.append('Content-Type', 'application/json');
        // return this.http.post('http://localhost:3000/admin/update-pwd', pwd, { headers: header }).map(res => res.json());
        return this.http.post('https://api.reatchall.com/admin/update-pwd', pwd, { headers: header }).map(res => res.json());
    }
