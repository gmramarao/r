...
export class BusinessDashboardCoreComponent implements OnInit {
  ...
  get_business_status(){
    ...
    this.vendorService.getBusinessStatus(this.b_id).subscribe((res)=>{
      console.log(res);
      if(res.success){
        this.status = res.msg.status === 'open' ? true: false;
      }
    })
  ...
  //         //   }
  //         // });
  //         if(bus.msg[1].business.days && bus.msg[1].business.days[this.today]){
