...
router.post('/register', function(a, b) {
    ...
    User.find({ mobile: mobile }, (err, found) => {
        if (err){
            res.json({ success: false, msg: err });
        }else {
            if(found.length > 0){
                res.json({ success: false, msg: 'Mobile already exists' });
            }else {
                // find email
                User.find({ email: email }, (er, f) => {
                    if(er) {
                        b.json({ success: false, msg: e });
                    }else {
                        if(f.length > 0 ){
                            b.json({ success: false, msg: 'Email already exists' });
                        }else {
                            var j = new User({
                                name: a.body.name,
                                email: a.body.email,
...
router.get('/find-email/:email', function(a, b) {
    ...
    //     })
    // })
    User.findOne({email:a.params.email}, function(err, doc){
        if(doc) {
            b.json({success: true,msg: 'User Found'});
        } else {
            b.json({success: false,msg: 'No user found'});
        }
    });
}); 
...
router.get('/find-mobile/:mobile', function(a, b) {
    ...
    //     })
    // })
    User.findOne({mobile: a.params.mobile}, function(err, doc){
        if(doc){
            b.json({success: true, msg: 'User Found'})
        } else {
            b.json({success: false, msg: 'No user found'})
...
router.get('/get-cat-from-name/:name', (req,res) => {
    ...
    Category.find({name: name},(err,found) => {
        if(err) {
            res.json({success: false, msg: err});
        }else{
            res.json({success: true, msg: found});
...
router.get('/get-sub-cats-of-cat/:id', (req,res) => {
    var id = req.params.cat_id;
    SubCategory.find({category_id: id},(err,found) => {
        if(err) {
            res.json({success: false, msg: err});
        }else{
            res.json({success: true, msg: found});
...
router.post('/post-visit-to-business', (req,res) => {
    ...
    BusinessVisitCount.find({ business_id: business_id }, (error, f) => {
        if (error) {

        }else {
            if ( f.length > 0) {
                var num = parseInt(f[0].number_of_visits) + 1;
                BusinessVisitCount.findOneAndUpdate({ business_id: business_id }, { number_of_visits: num, last_visited_time: visited_time }, (errr, upd) => {
                    if (errr) {
    ...
    BusinessVisit.find({ user_id: user_id, business_id: business_id }, (err, found) => {
        if (err) {
            console.log(err);
        }else {
            if (found.length < 1 ) {
                ...
                BusinessVisit.findOneAndUpdate({ user_id: user_id, business_id: business_id }, { number_of_visits: num, last_visited_time: visited_time }, (er, updated) => {
                    if (er) {
                        res.json({ success: false, msg: er });
                    }else {
...
router.post('/check-business-in-wishlist',(req,res) => {
    var user_id = req.body.user_id;
    var business_id = req.body.business_id;
    BusinessWishlist.find({user_id: user_id, business_id: business_id}, (err, found) => {
        if(err) {
            res.json({ success: false, msg: err });
        }else {
            res.json({ success: true, msg: found });
...
router.post('/check-item-in-wishlist',(req,res) => {
    ...
    ItemsWishlist.find({user_id: user_id, item_id: item_id}, (err, found) => {
        if(err) {
            res.json({ success: false, msg: err });
        }else {
            res.json({ success: true, msg: found });
...
router.post('/remove-business-in-wishlist',(req,res) => {
    user_id = req.body.user_id;
    business_id = req.body.business_id;
    BusinessWishlist.remove({ user_id: user_id, business_id: business_id }, (err, removed) => {
        if(err) {
            res.json({ success: false, msg: err });
        }else {
...
router.post('/get-user-business-wishlist/:user_id', (req,res) => {
    ...
    BusinessWishlist.find({user_id: user_id}, (err, found) => {
        if(err) {
            res.json({ success: false, msg: err });
        }else {
            res.json({ success: true, msg: found });
...
router.get('/get-cart/:user_id', (req,res) => {
    ...
    UserCart.find({ user_id: user_id },(err,cart) => {
        if (err) {
            res.json({ success: false, msg: err });
        }else {
...
// Add to cart
 router.post('/add-to-user-cart', (req,res) => {
    ...
    UserCart.find({user_id: user_id}, (err, found) => {
        if (err) {
            res.json({ success: false, msg: err });
        }else {
            ...
            if (found.length > 0) {
                // Insert into cart
                UserCart.findOneAndUpdate({ user_id: user_id }, {$push: { cart : item_id }, last_updated: moment().format('MMMM Do YYYY, h:mm:ss a') }, (er, updated) => {
                    if (er) {
                        if (er) {
                            res.json({ success: false, msg: er });
                        }else {
...
router.post('/remove-from-cart', (req,res) => {
    ...
    // res.json({ msg: 'hi' });
    UserCart.update( {user_id: user_id}, { $pullAll: {cart: [item_id] } }, (err, stat) => {
        if ( err ) {
            console.log(err);
            res.json( { success: false, msg: err } );
        }else {
            Item.update({_id: item_id},{cart_status: cart_status},{ upsert: true}, function(err, doc){
                res.json( { success: true, msg: stat } );
            })
        }
    });
});
...
router.post('/post-order', function(req, res){
    ...
    Order.insertMany({user_id : req.body.user_id, order_id : order_id, orders:req.body.orders, confirmation:'pending'}, function(err, doc){
        if (err) {
            res.json({ success: false, msg: err });
        }else {
            Item.update({_id:{$in:req.body.id}, cart_status:true},{ $set: { cart_status:false } },{multi: true}, function(err, doc1){
                console.log(doc1);
...
router.get('/get-confirmation/:id', function(req, res){
    Order.findOne({_id:req.params.id}, function(err, confr){
        if (err) {
            res.json({ success: false, msg: err });
        }else {
            res.json({ success: true, msg: confr });
        }
    })
})
router.put('/set-vendorRes', function(req, res){
    Order.update({_id:req.body.id, confirmation:req.body.current_status},{$set:{confirmation:req.body.modify_status}}, function(err, doc){
        if (err) {
            res.json({ success: false, msg: err });
        }else {
            res.json({ success: true, msg: doc });
